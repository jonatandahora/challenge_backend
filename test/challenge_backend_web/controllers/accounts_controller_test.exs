defmodule ChallengeBackendWeb.AccountsControllerTest do
  ...
  describe "sign up" do
    test "renders user_account when data is valid", %{conn: conn} do
      conn = post(conn, ~p"/api/accounts", @sign_up_attrs)
      assert json_response(conn, 201)["data"]
    end

    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, ~p"/api/accounts", @invalid_sign_up_attrs)
      assert json_response(conn, 422)["errors"] != %{}
    end
  end

  describe "login" do
    test "successfull with valid credentials", %{conn: conn} do
      user_account = %{id: user_account_id} = user_account_fixture(%{password: "123456"})

      conn = post(conn, ~p"/api/accounts/login", %{cpf: user_account.cpf, password: "123456"})
      assert token = json_response(conn, 200)["data"]["token"]

      assert {:ok, %{"sub" => ^user_account_id}} = decode_and_verify(token)
    end

    test "failed with invalid password", %{conn: conn} do
      user_account = user_account_fixture(%{password: "123456"})

      conn = post(conn, ~p"/api/accounts/login", %{cpf: user_account.cpf, password: "654321"})
